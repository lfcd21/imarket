module ApplicationHelper
  ...
  def release_notes
    <<~EOS.html_safe
      （5/5）<a href="/highest_forecasts">過去最高益予想銘柄</a> を表示するようにしました。<br>
      （4/28）<a href="/eps_estimates">日経平均</a> と <a href="/eps_estimates?tab=dow">ダウ平均</a> の予想EPSを表示するようにしました。
    EOS
  end
  ...
  def link_to_taiho(model, full = false)
    "<a class=#{link_class_name('taiho', full)} href='https://maonline.jp/db/shareholding_reports?isname_or_issyokencode_or_company_iscode_start=#{model.code}' target='_blank'>\
